...
def export_graph_paired(model_name, XtoY=True):
  graph = tf.Graph()
  ...
  with graph.as_default():
    ...
    paired_gan.model()
    if XtoY:
      output_image = paired_gan.sampleX2Y(tf.expand_dims(input_image, 0))
    else:
      output_image = paired_gan.sampleY2X(tf.expand_dims(input_image, 0))
...
def main(unused_argv):
  ...
  export_graph_paired(FLAGS.XtoY_model, XtoY=True)
  print('Export YtoX model...')
  export_graph_paired(FLAGS.YtoX_model, XtoY=False)
