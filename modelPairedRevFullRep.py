...
class PairedGANDisenRevFullRep:
  def __init__(self,
               XY_train_file='',
               batch_size=1,
               image_size=32,
  ...
  def model(self):
    ...
    fake_ex_y = self.Gdex(tf.concat([rep_Sx,rep_Ex],3))
    #fake_ex_y = self.Gdex(rep_Sx,rep_Ex)
    rev_X_loss = self.generator_loss(self.Dex_Y, fake_ex_y, use_lsgan=self.use_lsgan)
    ...
    Dex_Y_loss = self.discriminator_loss(self.Dex_Y, y, self.fake_ex_y, use_lsgan=self.use_lsgan)
    ...
    # Reverse gradient layer as maximing gan loss from exclusive part
    fake_ex_x = self.Fdex(tf.concat([rep_Sy,rep_Ey],3))
    rev_Y_loss = self.generator_loss(self.D_X, fake_ex_x, use_lsgan=self.use_lsgan)
    ...
    Dex_X_loss = self.discriminator_loss(self.Dex_X, x, self.fake_ex_x, use_lsgan=self.use_lsgan)
    ...
    tf.summary.histogram('Dex_Y/fake', self.Dex_Y(self.Gdex(tf.concat([rep_Sx,rep_Ex],3))))
    tf.summary.histogram('Dex_X/true', self.Dex_X(x))
    tf.summary.histogram('D_X/fake',self.Dex_X(self.Fdex(tf.concat([rep_Sy,rep_Ey],3))))
    ...
    tf.summary.image('X/exclusive_rec',
                     utils.batch_convert2int(self.Gdex(tf.concat([rep_Sx,rep_Ex],3))))
    ...
    tf.summary.image('Y/exclusive_rec',
                     utils.batch_convert2int(self.Fdex(tf.concat([rep_Sy,rep_Ey],3))))



    return G_loss, D_Y_loss, Dex_Y_loss, F_loss, D_X_loss, Dex_X_loss, A_loss, DC_loss, fake_y, fake_x, fake_ex_y, fake_ex_x
    #return G_loss, D_Y_loss, F_loss, D_X_loss, A_loss, fake_y, fake_x

  def optimize(self, G_loss, D_Y_loss, Dex_Y_loss, F_loss, D_X_loss, Dex_X_loss, A_loss, DC_loss):
    ...
    Dex_Y_optimizer = make_optimizer(Dex_Y_loss, self.Dex_Y.variables, name='Adam_Dex_Y')
    F_optimizer =  make_optimizer(F_loss, [self.Fe.variables,
                                           self.Fd.variables,self.Fdex.variables], name='Adam_F')
    D_X_optimizer = make_optimizer(D_X_loss, self.D_X.variables, name='Adam_D_X')
    Dex_X_optimizer = make_optimizer(Dex_X_loss, self.Dex_X.variables, name='Adam_Dex_X')
    ...
    with tf.control_dependencies([G_optimizer, D_Y_optimizer, Dex_Y_optimizer, F_optimizer,
                                  D_X_optimizer, Dex_X_optimizer, A_optimizer, DC_optimizer]):
