...
class PairedGANDisen:
  ...
  def model(self):
    ...
    Gdex_loss = self.generator_loss(self.Dex_Y, fake_ex_y, use_lsgan=self.use_lsgan)
    ...
    Fdex_loss = self.generator_loss(self.Dex_X, fake_ex_x, use_lsgan=self.use_lsgan)
  ...
  def optimize(self, G_loss, D_Y_loss, Dex_Y_loss, F_loss, D_X_loss,
               Dex_X_loss, A_loss, Feat_loss, DC_loss):
    ...
    # Try with another type of optimizer for DC
    DC_optimizer = make_optimizer(DC_loss, [self.DC.variables, self.Ge.variables, self.Fe.variables], name='Adam_DC')

    with tf.control_dependencies([G_optimizer, D_Y_optimizer, Dex_Y_optimizer, F_optimizer,
                                  D_X_optimizer, Dex_X_optimizer, A_optimizer,
                                  Feat_optimizer, DC_optimizer]):
