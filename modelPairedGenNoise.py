...
from generatorNoise import GeneratorNoise
...
class PairedGANDisen:
  def __init__(self,
    ...
    self.Gnoise = GeneratorNoise('Gnoise', self.is_training, norm=norm, image_size=image_size)
    # maybe we need a smaller discriminator for this problem?
    self.D_Gnoise = Discriminator('D_Gnoise', self.is_training, norm=norm, use_sigmoid=use_sigmoid)
  ...
  def model(self):
    ...
    Gnoise_loss = self.generator_loss(self.D_Gnoise, fake_rep_Ex, use_lsgan=self.use_lsgan)
    D_Gnoise_loss = self.discriminator_loss(self.D_Gnoise, rep_Ex, fake_rep_Ex, use_lsgan=self.use_lsgan)
    ...
    tf.summary.image('X/generated2',
                     utils.batch_convert2int(self.Gd(tf.concat([rep_Sx, fake_rep_Ex],3))))
  ...
  def optimize(self, G_loss, D_Y_loss, F_loss, D_X_loss, A_loss, Feat_loss, DC_loss, Gnoise_loss, D_Gnoise_loss):
    ...
    Gnoise_optimizer = make_optimizer(Gnoise_loss, self.Gnoise.variables, name='Adam_Gnoise')
    D_Gnoise_optimizer = make_optimizer(D_Gnoise_loss, self.D_Gnoise.variables, name='Adam_D_Gnoise')

    with tf.control_dependencies([G_optimizer, D_Y_optimizer, F_optimizer,
                                  D_X_optimizer, A_optimizer, DC_optimizer,
                                 Gnoise_optimizer, D_Gnoise_optimizer]):
