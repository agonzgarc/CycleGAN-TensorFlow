...
class GeneratorDecoderReversalFullRep:
  ...
  def __call__(self, inputS, inputE):
    """
    Args:
    ...
    with tf.variable_scope(self.name):
        ...
        #with g.gradient_override_map({"Identity": "ReverseGrad"}):
            # Stop gradient on shared rep, invert on exclusive rep
            #idinputS = tf.stop_gradient(inputS)
            #idinputE = tf.stop_gradient(inputE)
            #idinput = tf.concat([idinputS, idinputE],3)
            idinput = tf.concat([inputS, inputE],3)
